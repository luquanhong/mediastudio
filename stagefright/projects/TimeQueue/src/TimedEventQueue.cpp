...
void TimedEventQueue::cancelEvents(
    ...
    while (it != mQueue.end()) {
        ...
        printf("cancelling event %d", (*it).event->eventID());
...
void TimedEventQueue::threadEntry() {
    ...
    for (;;) {
...

        {
            ...
            while (mQueue.empty()) {
            	printf("the function %s mQueueNotEmptyCondition.wait(mLock):\n",__func__);
            ...
            for (;;) {
                ...
                if (delay_us > kMaxTimeoutUs) {
                    //printf("delay_us exceeds max timeout: %lld us", delay_us);
...
TimedEventQueue::Event* TimedEventQueue::removeEventFromQueue_l(
    ...
    printf("Event %d was not found in the queue, already cancelled?", id);
